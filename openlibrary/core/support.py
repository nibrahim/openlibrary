...
class Support(object):
    ...
    def get_all_cases(self, typ = "all", summarise = False, sortby = "lastmodified", desc = "true", user = False, staleok = False):
        "Return all the cases in the system"
        args = dict()
        if summarise:
            d = defaultdict(lambda: 0)
            if user:
                args.update(startkey = [user, "closed"], endkey = [user, {}])
            else:
                args.update(startkey = [None, "closed"], endkey = [None,{}])
            if staleok:
                args["stale"] = "ok"
